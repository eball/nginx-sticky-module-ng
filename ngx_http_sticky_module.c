...
static ngx_int_t ngx_http_init_sticky_peer(ngx_http_request_t *r, ngx_http_upstream_srv_conf_t *us) {
    ...
    if (iphp->request->headers_in.user_agent->value.len > 0 &&
        ...
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "[sticky/init_sticky_peer] safri cookie forbidden %ui", iphp->selected_peer);
        return NGX_OK;
    }
...
static ngx_int_t ngx_http_sticky_header_filter(ngx_http_request_t *r) {
    ...
    for (i = 0; /* void */; i++) {
        ...
        if (ngx_strncasecmp(elt[i].key.data, (u_char *)"set-cookie", 10) == 0 && ngx_strncasecmp(elt[i].value.data, ctx->sticky_conf->cookie_name.data, ctx->sticky_conf->cookie_name.len) == 0 && elt[i].value.data[ctx->sticky_conf->cookie_name.len] == '=') {
            elt[i].hash = 0;
        }
    }
